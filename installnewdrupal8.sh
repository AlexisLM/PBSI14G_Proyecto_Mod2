...
echo "Retrieving Drupal 7 site enabled modules..."
cd "${sitepath}"
...
sshpass -p "${webpass}" scp "new_${sitedir}.tar.gz" "${webuser}@${webhost}:~/"
echo "Transfering .conf file (VirtualHost)..."
sshpass -p "${webpass}" scp "${apachehost}" "${webuser}@${webhost}:~/"
echo "Extracting/Moving Drupal 8 site to new location..."
sshpass -p "${webpass}" ssh -o StrictHostKeyChecking=no -t "${webuser}@${webhost}" \
    ...
    mv new_${sitedir} /var/www/ && mv /var/www/new_${sitedir} /var/www/${sitedir}'"
